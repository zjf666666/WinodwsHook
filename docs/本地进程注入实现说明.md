...
public:
    ...
    // 策略管理
    bool CreateInjectStrategy(InjectionMethod method); // 预创建策略实例
    std::vector<InjectionMethod> GetSupportedMethods() const; // 获取支持的注入方法
...
private:
    ...
    // 策略缓存管理
    std::map<InjectionMethod, std::unique_ptr<IInjectionStrategy>> m_strategies;
    std::mutex m_strategyMutex; // 确保线程安全
};
```
...
// 使用不同的注入方法
InjectionResult result2 = injectionManager.InjectDll(
    targetProcessId,
...
1. **策略池管理**：在ProcessInjectionManager中维护一个策略对象池（m_strategies），可以同时存在多种注入策略实例
2. **方法参数化**：InjectDll方法接收InjectionMethod参数，支持动态选择注入方式
3. **延迟创建**：策略对象在首次需要时创建，并缓存以供后续使用
4. **线程安全**：使用互斥锁确保在多线程环境下策略创建和访问的安全性
