...
​	原子操作对于单条指令而言是线程安全的，将多条指令合并为一条指令执行以确保该指令的线程安全，例如：
...
// 添加原子锁后
LOCK XADD DWORD PTR [ATOMIC_I], 1
```

​	这种方式确保了单条语句的线程安全，但是在多条语句组合时，依旧是线程不安全的，例如```if (a) ++a; else --a;```，单条语句，如```if(a)```或是```++a```、```--a```都是线程安全的，但是他们之间可能会出现别的线程读写的操作，例如```if(a)```执行完成后，另一个线程执行了```--a```，此时我们又执行了一次```--a```得出的结果就是错误的了。
